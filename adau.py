...
from byteutils import wordToBytes
import time
...
class ADAU1761(dspi2c.DSPI2C):
    ...
    def checkPLLLocked(self):
        ...
        return bool(r1[5] & 0x02)

    def writePLLCtrl(self):
        r1 = wordToBytes(self.pllCtrlWord, 6)
...
def main():
    dsp = ADAU1761(0x3b)
    dsp.writeField("DSPEN", False)
    dsp.writeField("DSPRUN", False)
    dsp.showReg(0x4000)
    print("PLL Locked: {}".format(dsp.checkPLLLocked()))
    dsp.writeField("CLKSRC", True)
    dsp.writeField("INFREQ", 0x03)
    dsp.writePLLCtrl()
    print("PLL Locked: {}".format(dsp.checkPLLLocked()))
    dsp.writeField("COREN", True)
    dsp.writeField("CLK0", True)
    dsp.writeField("CLK1", True)
    dsp.writeField("MX1AUXG", 0x05)
    dsp.writeField("MX2AUXG", 0x05)
    dsp.writeField("MX1EN", True)
    dsp.writeField("MX2EN", True)
    dsp.writeField("ADCEN", 0x03)
    dsp.writeField("MX3LM", True)
    dsp.writeField("MX3EN", True)
    dsp.writeField("MX4RM", True)
    dsp.writeField("MX4EN", True)
    dsp.writeField("LHPVOL", 0x39)
    dsp.writeField("RHPVOL", 0x39)
    dsp.writeField("HPEN", True)
    dsp.writeField("HPMODE", True)
    dsp.writeField("LHPM", True)
    dsp.writeField("RHPM", True)
    dsp.writeField("PREN", True)
    dsp.writeField("PLEN", True)
    dsp.writeField("DACEN", 0x03)
    dsp.writeField("DSPEN", True)
    dsp.writeField("DSPSR", 0x0F)

    dsp.showReg(0x4003, 0x30)

    program = [] 

    dsp.writeReg(0x0800, program)
    
    param = [0x00, 0x00, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x02, 0xAA, 0xAE, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00]
    dsp.writeReg(0x0000, param)

    dsp.writeField("DSPRUN", True)
    dsp.writeField("DSPSR", 0x01)
