...
def main():
    ...
    with prog_file as open("program"):
        progdata = prog_file.read()

    program = list(map(lambda b: int(b.strip(), 16), progdata.split(',')[:-1]
    print(program)

    dsp.writeReg(0x0800, program)

    with param_file as open("param"):
        paramdata = param_file.read()

    param = list(map(lambda b: int(b.strip(), 16), paramdata.split(',')[:-1]
    print(param)
